...
export async function POST(req: NextRequest) {
  try {
    ...
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (data.id) {
        // Update existing experience
        createdExperience = await tx.experience.update({
          where: { id: data.id },
          data: experienceData,
        });
      } else {
        // Create new experience
        createdExperience = await tx.experience.create({
          data: experienceData,
        });
      }
    });
    ...
    return NextResponse.json({ success: true, experience: createdExperience });
  } catch (error) {
    console.error("Failded to add experience", error);
    return handleServerError(error);
  }
